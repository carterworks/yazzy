...
export const GET: APIRoute = async ({ request }) => {
	...
	log(`[requestId=${requestId}] db-dump: Starting. Memory: ${getMemoryUsageMB()}`);

	const dbPath = process.env["DB_PATH"];

	// If using in-memory database, fall back to serialize (dev only)
	if (!dbPath || dbPath === ":memory:") {
		log(`[requestId=${requestId}] db-dump: Using in-memory serialize (dev mode)`);
	...
	try {
		...
		log(`[requestId=${requestId}] db-dump: WAL checkpoint complete. Memory: ${getMemoryUsageMB()}`);

		const file = Bun.file(dbPath);
		const exists = await file.exists();
		if (!exists) {
			log(`[requestId=${requestId}] db-dump: Database file not found at ${dbPath}`);
			return new Response("Database file not found", { status: 500 });
		}

		const fileSize = file.size;
		log(`[requestId=${requestId}] db-dump: Streaming ${Math.round(fileSize / 1024 / 1024)}MB file from disk`);
